...
def _compute_bbe_for_file(task):
    """Compute thermochemistry for a single file (parallel-safe helper)."""
    (file, options, cosmo_option, ssymm_option, vmm_option) = task
    ...
    if options.D3 or options.D3BJ:
        ...
        damp = 'zero' if options.D3 else 'bj'
        if options.ATM:
            abc_term = True
        try:
...
def main():
    ...
    parser.add_argument("-njob", "--njob", dest="njob", type=int, default=1,
                        help="Number of subprocesses to use when processing multiple files (default 1).")
    # Parse Arguments
    ...
    for file in files:
        if options.cosmo:
            ...
            cosmo_option = None
        tasks.append((file, options, cosmo_option, ssymm_option, vmm_option))

    if len(tasks) > 1 and njob > 1:
        with multiprocessing.Pool(processes=min(njob, len(tasks))) as pool:
            results = pool.map(_compute_bbe_for_file, tasks)
    else:
        results = [_compute_bbe_for_file(task) for task in tasks]

    for _, bbe, d3_failed in results:
        if d3_failed:
